export default class SettingsView {
	...
	addProfileEventListeners() {
		...
		newName.addEventListener("keydown", async (event) => {
			if (event.key === "Enter") {
				try {
					...
					} else throw new Error("Failed to change the display name");
				} catch (error) {
		...
		fileInput.addEventListener("change", async (e) => {
			...
			try {
				...
				if (data.success) this.message(true, "Avatar changed!");
	...
	addSecurityEventListeners() {
		...
		wipeBtn.addEventListener("click", () => {
			if (this.eraseInDB()) window.app.logout();
		});
	}
	...
	async eraseInDB() {
		try {
			...
			if (data.success) alert("deleted user successfully");
			else throw new Error(data.message);
		} catch (error) {
	...
	async saveChanges(main) {
		try {
			...
			if (data.success) {
				...
				if (!main) this.message(true, "Theme and quality changes saved!");
				else {
					window.app.router.navigateTo("/index");
