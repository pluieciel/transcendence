...
class GameConsumerInvite(AsyncWebsocketConsumer):
	...
	async def connect(self):
		...
		recipent = query_params.get("recipent", [None])[0]
		if not token:
			return
		...
		if not user:
			...
			return
		if sender: # msg from B, A invite B, sender is A
			...
			if user.is_playing or await self.get_user(sender).is_playing:
				...
				return
			#TODO: a game table in DB for game management, with id, player_left, player_right, game_mode, game_type(AI, quick, invite)
